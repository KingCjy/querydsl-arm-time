        ...
        return new BasicExtractor<X>( javaTypeDescriptor, this ) {
            ...
            protected X doExtract(ResultSet rs, String name, WrapperOptions options) throws SQLException {
                if(options.getJdbcTimeZone() != null) {
                    return javaTypeDescriptor.wrap( rs.getTimestamp( name, Calendar.getInstance( options.getJdbcTimeZone() ) ), options );
                } else {

                    Timestamp timestamp = rs.getTimestamp(name);
                    System.out.println("extractor timestamp : " + timestamp);

                    X wrap = javaTypeDescriptor.wrap(rs.getTimestamp(name), options);
                    System.out.println("extractor wrap : " + wrap);
                    return wrap;
                }
            }
